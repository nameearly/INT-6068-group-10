...
#### 核心子项目划分
1. **PyBullet物理仿真（核心/最完整）**：基于PyBullet物理引擎构建3D仿真环境，实现无人机路径规划的全流程（感知、决策、执行），支持PPO算法训练、大规模并行训练、仿真可视化等，是项目的核心实现；
...
该模块是项目最完整的实现，所有操作均在「基于pybullet的仿真模拟训练/pybullet 版 7.21/」目录下执行：
- **运行仿真可视化（加载预训练模型）**：
  ```bash
  cd 基于pybullet的仿真模拟训练/pybullet 版 7.21/
