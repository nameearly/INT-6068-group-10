### 一、项目概述
该项目是**INT6068 - 神经网络与深度学习**课程的学术研究项目，核心目标是基于强化学习（RL）实现3D无人机的自主路径规划，包含三个迭代演化的独立子项目，整体采用模块化设计，结合~~物理仿真~~、强化学习算法与深度学习框架完成无人机路径规划的训练、仿真与验证：

> 💡 思考题：为什么我们要在无人机自主路径规划中使用强化学习？
...
#### 技术债务和演进逻辑：
`pybullet版本` 是一个简单的奖励系统，主要关注目标达成和碰撞避免，后续开发中遇到了多智能体扩展的复杂性，因此做了 `静态模拟/结合体.py` 放弃了物理真实性，专注于 #多智能体 #强化学习 算法研究
...
- **增强版TD3训练（开发中）**：
  ...
  ```
#### 2. 启动简化仿真环境
简化仿真包含3个变体，入口为各变体目录下的`结合体.py`，路径在「简化仿真模拟环境下的结果/」。**因为可能有调参需求，当前所有代码已修改为纯规则-based启发式算法的空白对照组，没有加载和使用RL训练模型。：**
...
### Future development
 目标是将此GitHub 仓库打造成：环境 Gym 化、参数配置化、模型有对比、结果有动图、复现有指南。
 无论对于学习者还是未来的面试官来说，这都会是一个极具含金量和专业素养的优秀 RL 项目！
